...
let device = null;
let sgemmNaivePipeline = null;
let sgemmNaiveCoalescedPipeline = null;
let sgemmBlockedPipeline = null;
let sgemmVectorizedPipeline = null;
let sgemmBindGroupLayout = null;
...
async function initSGEMMPipeline() {
    if (!device)
...
async function run_test() {
    const shapes = parseMNKTuples(document.getElementById('test_shapes').value);
    const alpha = 1; //1.0;
...
async function sgemm1(m, n, k, alpha, array_a, array_b, runtime) {
    if (!device) {
        throw new Error('WebGPU device not available');
    }
...
async function sgemm2(m, n, k, alpha, array_a, array_b, runtime) {
    if (!device) {
        throw new Error('WebGPU device not available');
    }
    // Create buffers - we need 5 buffers total:
...
async function sgemm3(m, n, k, alpha, array_a, array_b, runtime) {
    if (!device) {
        throw new Error('WebGPU device not available');
    }
    // Create buffers - we need 5 buffers total:
...
async function sgemm4(m, n, k, alpha, array_a, array_b, runtime) {
    if (!device) {
        throw new Error('WebGPU device not available');
    }
    // Create buffers - we need 5 buffers total:
